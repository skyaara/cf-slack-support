...
import { mediaKeyFromPath } from '@cf-slack-support/media';
import { verifySlackSignature } from '@cf-slack-support/slack';
...
export function defineSlackSupport<Env extends object>(
  ...
  app.get(health, (c) => c.json({ ok: true, features: features.map((f) => f.name) }));
